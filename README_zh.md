...
| 你的情况 | 选 | 要填什么 |
|----------|----|----------|
| 想最快能用，这台机器有管理员权限 | 启动菜单 **[1]** Cloudflare WARP | 不用填，程序自己开好代理模式（SOCKS5，只给 TG 走） |
| 已经有 Clash / 机场 / VPS 代理 | 启动菜单 **[2]** | 代理地址，如 `http://127.0.0.1:7890` |
| **有自己的域名（Cloudflare），不想装任何东西** | 启动菜单 **[3]** 自建反代 | 你的 Worker 地址 —— 看下面「前缀式 / 路径式」怎么填 |
| 暂时不想折腾 | 启动菜单 **[4]** 跳过 | 不用填；抢票照常，只是收不到通知和指令 |

> 三种方案**都只影响 Telegram**：KTMB 抢票、浏览器、远程控制始终直连，速度不受影响。
> 配置随时可在面板「通知」页改，点一下「测试代理」就知道通不通（通过会自动把地址回填成完整形式）。

### 自建反代怎么填：前缀式 vs 路径式

看你的 Worker 是哪种写法（不确定就两种都点一次「测试代理」，哪个 ✅ 用哪个）：

| 你的 Worker | 实际访问地址长这样 | 「Telegram API 地址」填 |
|--------------|--------------------|--------------------------|
| **前缀式**（把完整地址接在前缀后面） | `https://worker.你的域名.com/https://api.telegram.org/bot<token>/getMe` | **只填 `https://worker.你的域名.com/`**，程序会自动补上 `/https://api.telegram.org` |
| **路径式**（固定域名 + 密钥路径） | `https://tg.你的域名.com/密钥/bot<token>/getMe` | `https://tg.你的域名.com/密钥`（原样使用） |

> 判断规则：**只填域名（没有路径）= 前缀式；带路径 = 路径式**。两种都支持，填错也不会坏 —— 测试会告诉你。
